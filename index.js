...
// end include: /tmp/tmp1tqq55gs.js
// include: /tmp/tmpkgbizbhn.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if (Module['$ww'] || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpkgbizbhn.js
// include: /tmp/tmpzhbcjbe0.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach(function(task) {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpzhbcjbe0.js
...
var ASM_CONSTS = {
  131638: () => { if (document.fullscreenElement) return 1; },  
 131684: () => { return Module.canvas.width; },  
 131716: () => { return parseInt(Module.canvas.style.width); },  
 131764: () => { document.exitFullscreen(); },  
 131791: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 131864: () => { if (document.fullscreenElement) return 1; },  
 131910: () => { return Module.canvas.width; },  
 131942: () => { return screen.width; },  
 131967: () => { document.exitFullscreen(); },  
 131994: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 132127: () => { return window.innerWidth; },  
 132153: () => { return window.innerHeight; },  
 132180: () => { if (document.fullscreenElement) return 1; },  
 132226: () => { return Module.canvas.width; },  
 132258: () => { return parseInt(Module.canvas.style.width); },  
 132306: () => { if (document.fullscreenElement) return 1; },  
 132352: () => { return Module.canvas.width; },  
 132384: () => { return screen.width; },  
 132409: () => { return window.innerWidth; },  
 132435: () => { return window.innerHeight; },  
 132462: () => { if (document.fullscreenElement) return 1; },  
 132508: () => { return Module.canvas.width; },  
 132540: () => { return screen.width; },  
 132565: () => { document.exitFullscreen(); },  
 132592: () => { if (document.fullscreenElement) return 1; },  
 132638: () => { return Module.canvas.width; },  
 132670: () => { return parseInt(Module.canvas.style.width); },  
 132718: () => { document.exitFullscreen(); },  
 132745: ($0) => { Module.canvas.style.opacity = $0; },  
 132783: () => { return screen.width; },  
 132808: () => { return screen.height; },  
 132834: () => { return window.screenX; },  
 132861: () => { return window.screenY; },  
 132888: () => { return window.devicePixelRatio; },  
 132924: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 132977: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 133028: () => { Module.canvas.style.cursor = 'none'; },  
 133065: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 133321: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 133372: () => { if (document.fullscreenElement) return 1; },  
 133418: () => { return window.innerWidth; },  
 133444: () => { return window.innerHeight; },  
 133471: () => { if (document.pointerLockElement) return 1; },  
 133518: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 135676: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 135840: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 135944: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 138943: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 139009: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 139402: ($0) => { miniaudio.untrack_device_by_index($0); },  
 139445: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 139570: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
